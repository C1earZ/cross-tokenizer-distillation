...
def tokenize(item, tokenizer, encoder_decoder=False):
    """将医学问答数据项转换为模型输入的token序列
    参数:
        item: 数据项，包含context（医学论文）、question（问题）、answers_generated（生成的答案）
        tokenizer: 分词器对象
    ...
    if tokenizer.name_or_path == "meta-llama/Llama-2-7b-chat-hf":
        ...
        shot = 3  # Mistral-Instruct使用3个示例
    elif tokenizer.name_or_path == "tiiuae/falcon-7b-instruct":
        shot = 3  # Falcon-instruct使用3个示例
...
# 定义函数：获取指定划分的数据集
def get_split(dataset_config, tokenizer, split, size=1):
    """加载并预处理PubMed QA数据集的指定划分
    参数:
        dataset_config: 数据集配置对象
        tokenizer: 分词器
        split: 数据集划分（'train'或'validation'）
        size: 数据集大小比例（默认为1，即使用全部数据）
    返回:
        处理后的数据集对象"""
    # 从磁盘加载预生成的PubMed QA数据集
    # 数据集名称格式：{生成模型名称}-pubmed_qa_50k
    dataset = load_from_disk(f"{os.getenv('HOME')}/llm-distillation/datasets/hf/{dataset_config.generated_by.split('/')[-1]}-pubmed_qa_50k")
    # 选择指定的数据集划分
